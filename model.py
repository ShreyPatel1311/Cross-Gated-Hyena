...
class LongConv(nn.Module):
    ...
    HyenaFilter design (standalone_hyena.py), but conditioned on physical bond
    distances via RBF encoding rather than sinusoidal position embeddings —
    preserving the geometric inductive bias of the original model.

    Convolution is computed with fftconv() from standalone_hyena.py, which
    implements linear convolution with a learnable per-channel diagonal bias D
    (residual skip connection, same as in the canonical HyenaFilter.forward).
    ...
    def __init__(self, num_rbf_pos: int, seq_dim: int, filter_hidden: int = 64):
        super().__init__()
        # Implicit filter MLP — Sin activation from HazyResearch/safari,
        # matching HyenaFilter.implicit_filter but conditioned on dist_rbf
